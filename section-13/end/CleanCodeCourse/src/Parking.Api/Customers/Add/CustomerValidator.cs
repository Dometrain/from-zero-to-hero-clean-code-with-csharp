...
internal class CustomerValidator
{
    ...
    public bool IsValid(Customer customer)
    {
        ...
        if (IsOverThenLicensePlateLimit(customer))
	        return false;

        return true;
    }

    private bool IsOverThenLicensePlateLimit(Customer customer)
    {
	    var licensePlateNumberLimit = customer.IsEmployee ? GetMaxAllowedLicencePlatesForEmployee()
		    : GetMaxAllowedLicencePlatesForRegularCustomer(customer.MembershipLevel);
	    return customer.VehicleLicensePlates.Count > licensePlateNumberLimit;
    }
